...
REGISTRY ?= docker.pkg.github.com/karthikcru/itachi
...
SRC_DIRS := itachi # directories which hold app source (not vendored)
...
DOTFILE_IMAGE_TEST = $(subst /,_,$(IMAGE))-$(TAG_TEST)
...
.container-$(DOTFILE_IMAGE): bin/$(OS)_$(ARCH)/$(BIN) Dockerfile
	@sed                                 \
	    ...
	    Dockerfile > .dockerfile-$(OS)_$(ARCH)
	@docker build -t $(IMAGE):$(TAG) -t $(IMAGE):latest -f .dockerfile-$(OS)_$(ARCH) .
	@docker images -q $(IMAGE):$(TAG) > $@

container-test: 
	@docker build -t itachi_test -f DockerFile.test .
...
build_and_test: container-test test
test: $(BUILD_DIRS)
	@docker run                                                 \
	    -i                                                      \
	    --rm                                                    \
